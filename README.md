...
3. **実行方法**
   ...
   「実行」ボタンを押すと、コンソール画面が自動的に開き、収集処理が実行します。

   またエクスプローラーから「touki-scrapy.exe」をダブルクリックしても収集処理が実行できます。
...
4. **補足:**
   ...
   - スクリプト実行中のブラウザは手動操作しないでください。このブラウザを非表示にすることも可能です。不要な場合は「Condition.xlsm」の「設定値」シートの10行目の値を「1」に変更してください。

   - 進捗状況のWindows通知、ブラウザ表示を追加しました。不要な場合は「Condition.xlsm」の「設定値」シートの11,12行目の値を「0」に変更してください。

   - 地番・家屋番号の選択数の上限は「50」、地番・家屋番号のFromTo間隔の上限は「10」となっています。[登記情報提供サービス](https://www1.touki.or.jp/gateway.html) サイトの制限です。これらの上限値は「Condition.xlsm」の「設定値」シートの3,4行目で設定しています。将来、サイトの制限が緩和されたら、本値をあわせて変更してください。

...
[登記情報提供サービス](https://www1.touki.or.jp/gateway.html) の「不動産請求情報」ページをスクレーピングするためのPythonスクリプトです。スクレーピングに「selenium」、結果ファイルのExcel出力に「openpyxl」を、Windows通知に「plyer」を利用しています。
...
3. **実行方法**
   ...
   「Condition.xlsm」を開いて「ID番号」、「パスワード」を記入して保存してください。
   その他の条件は適当なものに変更してください。
   「実行」ボタンを押すと、コンソール画面が自動的に開き、収集処理を実行します。

   またエクスプローラーから「touki-scrapy.exe」をダブルクリックしても収集処理を実行できます。

4. **補足:**

   - [登記情報提供サービス](https://www1.touki.or.jp/gateway.html) サイトへのアクセス集中を回避するため、アクセス間隔は約3秒（正確には「3秒±20%」）以上となるように待機時間を設けています。なお、「3秒」は「Condition.xlsm」の「設定値」シートの8行目で、「±20%」は9行目で設定しており変更は可能です。

   - 「検索」ボタンをクリックなどで、新しい画面の表示する場合、その表示待ちの時間を最大「10秒」としています。この時間を越えるとタイムアウトエラーで処理中断します。なお、「10秒」は「Condition.xlsm」の「設定値」シートの5行目で設定しています。もし、タイムアウトエラーが頻発する場合は、この値を調整（大きく）することでタイムアウトエラーを解消してください。
   ...
   - 「地番・家屋番号」の検索時において、頻度が高く「再実行」を促されるページされ検索できないめ、検索の再実行します。ただし、5回連続して検索できない場合は、処理中断します。なお、再実行の間隔は「5秒」としています。「5回」は「Condition.xlsm」の「設定値」シートの7行目で、「5秒」は6行目で設定しており変更は可能です。
   ...
   - スクリプト実行中のブラウザは手動操作しないでください。このブラウザを非表示にすることも可能です。ブラウザを非表示にしたい場合は「Condition.xlsm」の「設定値」シートの10行目の値を「1」（headlessモード有効）に変更してください。

   - 進捗状況のWindows通知、ブラウザ通知機能を追加しました。この通知が不要な場合は「Condition.xlsm」の「設定値」シートの11,12行目の値を「0」（通知なし）に変更してください。

   - 地番・家屋番号の選択数の上限は「50」、地番・家屋番号のFromTo間隔の上限は「10」となっています。[登記情報提供サービス](https://www1.touki.or.jp/gateway.html) サイトの制限です。これらの上限値は「Condition.xlsm」の「設定値」シートの3,4行目で設定しています。将来、サイトの制限が緩和された際には、本値を変更してください。
